...
interface IUserModel extends mongoose.Model<IUser>{
  findIncompleteProfiles(): mongoose.Query< IUser[], IUser>
}
...
const UserSchema = new mongoose.Schema({
  ...
  dob: { 
    ...
    validate: {
      ...
      message: 'Please provide a valid date of birth (age between 13-120 years)'
    }
   },
  phone: { 
    type: String, 
    ...
    validate: {
      validator: function (phone: string) {
        return phone === null || /^[\+]?[1-9][\d]{0,15}$/.test(phone);
      },
...
UserSchema.statics.findIncompleteProfiles = function() {
  return this.find({ profileComplete: false, isActive: true });
};
